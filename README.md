...
**программы, которые не зависят от**:
- операционной системы `sys_*.c` для демонстрации на текущих осях
- видеокарты `Delay_ms(Fps)` просто константа
- процессора `32/64/128...` бит автоадаптация
- монитора `8k/16k/32k...` автоадаптация
- цветность `USE_BW/USE_RGB` автоадаптация
- страны `UTF8` полная поддержка без библиотек
- «экосистем» `NoLibs` лишнее
